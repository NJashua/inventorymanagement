...
#             cursor.execute("SELECT * FROM PRODUCT WHERE PRODUCT_NAME LIKE %s", (search_term,))
#             cursor.execute(""" SELECT p. *, l.* FROM PRODUCT p INNER JOIN LOCATIONS l ON p.LOCATION_ID = 1.LOCATION_ID
#                            WHERE p.PRODUCT_NAME LIKE %s""", (search_term,))
...
#             else:
#                 cursor.execute("""
#                     INSERT INTO PRODUCT (SUPPLIER_ID, SUPPLIER_NAME, PRODUCT_NAME, DESCRIPTION, NUMBER_OF_PRODUCT_PURCHASED, DATE_OF_PURCHASE, CATEGORY, QUANTITY_AVAILABLE, UNIT_COST, SELLING_PRICE, REORDER_LEVEL, LOCATION_ID)
#                     VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
#                 """, (supplier_id, supplier_name, product_name, description, num_products_received, date_of_purchase, category, num_products_received, unit_cost, selling_price, reorder_level, location_id))
...
#                 shortcut_details = f"""
#                     Quick instruction to locate "{product_name}":- Go to the {location_zone} & Find RAAK {location_aisle} &- Locate Shelf {location_shelf} in Aisle {location_aisle} &- Look for Bin {location_bin} in Shelf {location_shelf} & - Collect the product {product_name} from {location_bin}""".strip()
#                 response['shortcut_details'] = shortcut_details
...
#         except snowflake.connector.Error as e:
#             print("Getting error while deleting products..")
#         finally:
#             cursor.close()
#             connection.close()
...
#             product_name = data.get('product_name')
#             description = data.get('description', '')
#             number_of_product_purchased = data.get('number_of_product_purchased', 0)
#             date_of_purchase = data.get('date_of_purchase', '2000-01-01 00:00:00')
#             category = data.get('category', 'General')
#             quantity_available = data.get('quantity_available', 0)
#             unit_cost = data.get('unit_cost', 0)
...
#             if None in [purchase_id, supplier_id, product_name, number_of_product_purchased, date_of_purchase, unit_cost, reorder_level]:
#                 return jsonify({"error": "Missing required fields or fields cannot be NULL"}), 400
...
#                 column_names = [desc[0] for desc in cursor.description]
#                 product_dict = dict(zip(column_names, product))
#                 new_quantity = product_dict['quantity_available'] + number_of_product_purchased
#                 new_reorder_level = max(0, product_dict['reorder_level'] - number_of_product_purchased)
#                 cursor.execute("""
#                     UPDATE PRODUCT
#                     SET QUANTITY_AVAILABLE = %s, REORDER_LEVEL = %s, UNIT_COST = %s
#                     WHERE PRODUCT_NAME = %s
...
#             else:
#                 cursor.execute("""
#                     INSERT INTO PRODUCT (SUPPLIER_ID, SUPPLIER_NAME, PRODUCT_NAME, DESCRIPTION, NUMBER_OF_PRODUCT_PURCHASED, DATE_OF_PURCHASE, CATEGORY, QUANTITY_AVAILABLE, UNIT_COST, SELLING_PRICE, REORDER_LEVEL, EXPIRY_DATE, LOCATION_ID)
#                     VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
#                 """, (supplier_id, supplier_name, product_name, description, number_of_product_purchased, date_of_purchase, category, quantity_available, unit_cost, selling_price, reorder_level, expiry_date, location_id))

#             cursor.execute("""
#                 INSERT INTO PURCHASES (PURCHASEID, SUPPLIERID, DATEOFPURCHASE, NUMOFPRODUCTSRECEIVED, PRODUCT_NAME, CATEGORY)
#                 VALUES (%s, %s, %s, %s, %s, %s)
#             """, (purchase_id, supplier_id, date_of_purchase, number_of_product_purchased, product_name, category))

#             cursor.execute("""
#                 INSERT INTO SUPPLIERS (SUPPLIERID, SUPPLIERNAME, PRODUCT_NAME, CATEGORY, DELIVERY_DATE, DELIVERY_QUANTITY)
#                 VALUES (%s, %s, %s, %s, %s, %s)
#             """, (supplier_id, supplier_name, product_name, category, date_of_purchase, number_of_product_purchased))
#             cursor.execute("""
#                             INSERT INTO LOCATIONS(LOCATION_ID, AISLE, SHELF, BIN, ZONE)
#                            VALUES (%s, %s, %s, %s, %s)""", (location_id , location_aisle, location_shelf, location_bin, location_zone))
