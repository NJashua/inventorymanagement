...
class SnowflakeDB:
    ...
    def get_product_details_with_location(self, search_term):
        try:
            ...
            if products_list and location_list:
                ...
                Location = f"""
                    Quick instruction to locate "{product_name}": Go to the {location_zone} Zone & Find {location_aisle} rack & Locate Shelf {location_shelf} in Aisle {location_aisle} & Look for {location_bin} Bin in Shelf {location_shelf} & Collect the product {product_name} from {location_bin}""".strip()
                response['location'] = Location
    ...
    def delete_expired_products(self):
        try:
            ...
            cursor = connection.cursor()
            current_date = datetime.datetime.now().date()
            cursor.execute("DELETE FROM PRODUCT WHERE EXPIRY_DATE < ?", (current_date,))
            connection.commit()
            print("Products deleted successfully.")
        except snowflake.connector.Error as e:
            print(f"Error while deleting products: {str(e)}")
    ...
    def insert_data(self, data):
        try:
            ...
            if product:
                ...
                cursor.execute("""
                    UPDATE PRODUCT
                    SET QUANTITY_AVAILABLE = ?, REORDER_LEVEL = ?, UNIT_COST = ?
                    WHERE PRODUCT_NAME = ?
                """, (new_quantity, new_reorder_level, unit_cost, product_name))
            else:
                # If product does not exist, insert a new product
    ...
    def order_service_details(self, data):
        try:
            ...
            cursor.execute("SELECT QUANTITY_AVAILABLE FROM PRODUCT WHERE PRODUCT_NAME = ?", (product_name,))
            product = cursor.fetchone()
            if not product:
                return jsonify({"error": f"Product {product_name} not found"}), 404
            ...
            cursor.execute("UPDATE PRODUCT SET QUANTITY_AVAILABLE = ? WHERE PRODUCT_NAME = ?", (new_quantity, product_name))
